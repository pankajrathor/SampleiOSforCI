...
def xcodeproj = 'SampleiOSforCI.xcodeproj' // Path to the xcodeproj
def xcarchive_name = "SampleiOSforCI.xcarchive" // Name of the archive to build
def build_scheme = 'SampleiOSforCI' // Scheme to build the app
def test_scheme = 'SampleiOSforCI' // Scheme to build tests
    ...
    try {
        ...
        stage('Build') {
            wrap([$class: 'AnsiColorBuildWrapper', 'colorMapName': 'XTerm']) {
                ...
                sh "xcodebuild -scheme '${build_scheme}' -destination 'name=iPhone 8' clean build | tee build/xcodebuild.log | xcpretty"
